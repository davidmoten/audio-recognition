...
public class AudioTest {
    ...
    public void testReadUsingObservable() {
        int count = Audio.readSignal(AudioTest.class.getResourceAsStream("/alphabet.wav")).count()
                .toBlocking().single();
    ...
    public void testExtractMFCCs() {
        Audio.timeSeries(AudioTest.class.getResourceAsStream("/A.wav")).subscribe();
    }

    private static TimeSeries timeSeries(String resource) {
        return Audio.timeSeries(AudioTest.class.getResourceAsStream(resource)).toBlocking()
                .single();
    }

    private static double distance(TimeSeries a, TimeSeries b) {
        return FastDTW.compare(a, b, Distances.EUCLIDEAN_DISTANCE).getDistance();
    }
                ...
                for (List<Double> list : all) {
                    ...
                    for (double d : list) {
                        double prop = Math.max(0,
                                Math.min(1.0, (Math.log10(d) - min) / (max - min)));

...
public class Audio {
            ...
            public void call(Subscriber<? super Integer> sub) {
                try {
                    ...
                    while (!sub.isUnsubscribed()
                            && (bytesRead = audioInputStream.read(data, 0, data.length)) != -1) {
                        ...
                        for (int i = 0; i < n * 2; i += 2) {
                            int value = valueFromTwoBytesEndian(data[i], data[i + 1], isBigEndian);
    ...
    private static int valueFromTwoBytesEndian(int b1, int b2, boolean isBigEndian) {
        if (b1 < 0)
            b1 += 0x100;
        if (b2 < 0)
            b2 += 0x100;
    ...
    private static void printAudioDetails(AudioInputStream audioInputStream, AudioFormat audioFormat) {
